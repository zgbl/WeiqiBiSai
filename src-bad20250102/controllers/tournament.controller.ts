...
export class TournamentController {
  ...
  async getTournamentById(req: Request, res: Response) {
    try {
      const { id } = req.params;
      console.log('GET /api/tournaments/:id', { id });
      ...
      res.status(500).json({ 
        message: error instanceof Error ? error.message : 'Failed to get tournament' 
      });
    }
  }
  ...
  async deleteRound(req: Request, res: Response) {
    try {
      ...
      console.log('Deleting round:', { tournamentId: id, roundNumber });
      ...
      const roundIndex = parseInt(roundNumber) - 1; // 转换为0基索引
      if (isNaN(roundIndex) || roundIndex < 0 || roundIndex >= tournament.rounds.length) {
      ...
      if (roundIndex !== tournament.rounds.length - 1) {
        return res.status(400).json({ 
          message: 'Can only delete the last round of the tournament' 
        });
      }
  ...
  async removePlayer(req: Request, res: Response) {
    try {
      ...
      if (tournament.status !== 'UPCOMING') {
        return res.status(400).json({ message: 'Cannot remove players from an ongoing or completed tournament' });
      }

      // 从选手列表中移除选手
      tournament.players = tournament.players.filter(p => p.toString() !== playerId);
      await tournament.save();
  ...
  // Generate next round
  async generateRound(req: Request, res: Response) {
    try {
      const { tournamentId } = req.params;
      await this.tournamentService.generateRound(tournamentId);
  ...
  // Record match result
  async recordMatchResultNew(req: Request, res: Response) {
    try {
      const { matchId } = req.params;
      const { winnerId, isDraw } = req.body;

      await this.tournamentService.recordMatchResult(matchId, winnerId, isDraw);

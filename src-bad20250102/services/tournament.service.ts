...
export class TournamentService {
  ...
  calculatePlayerScores(tournament: any): Map<string, { score: number, opponentScore: number }> {
    ...
    tournament.players.forEach((player: any) => {
      scores.set(player._id.toString(), { 
        score: player.mcMahonScore || 0, 
        opponentScore: 0 
      });
    });
  ...
  private async generateNextRound(tournamentId: string): Promise<void> {
    ...
    console.log('Starting to generate next round...');
    const currentRound = tournament.rounds.length;
    console.log('Current round:', currentRound);
    console.log('Current tournament rounds:', tournament.rounds);

    // 按分数对选手进行分组
    const playersByScore = tournament.players.reduce((acc: { [key: number]: any[] }, player) => {
      const score = player.mcMahonScore || 0;
      acc[score] = acc[score] || [];
    ...
    for (const score of scores) {
      ...
      for (let i = 0; i < availablePlayers.length; i++) {
        ...
        for (let j = i + 1; j < availablePlayers.length; j++) {
          ...
          const havePlayed = tournament.rounds.some(round =>
            round.matches.some(match =>
              (match.player1._id.toString() === player1._id.toString() && match.player2._id.toString() === player2._id.toString()) ||
              (match.player1._id.toString() === player2._id.toString() && match.player2._id.toString() === player1._id.toString())
            )
          );

          if (rankDiff <= 3 && !havePlayed) {
            // 创建新的对局数据
            const matchData = {
              tournament: tournamentId,
        ...
        // 如果没有找到合适的对手，尝试在其他分数组中寻找
        if (!foundOpponent) {
          const player = availablePlayers[i];
          let opponent = null;

          // 向下寻找对手
          for (let searchScore = score - 1; searchScore >= 0; searchScore--) {
            if (!playersByScore[searchScore]) continue;
            
            opponent = await this.findSuitableOpponentInSameGroup(
              player,
              playersByScore,
              searchScore,
              pairedPlayers
          ...
          // 如果向下没找到，向上寻找
          if (!opponent) {
            for (let searchScore = score + 1; searchScore <= Math.max(...scores); searchScore++) {
              if (!playersByScore[searchScore]) continue;
    ...
    // 处理未配对的选手
    const unpairedPlayers = tournament.players
      .filter(p => !pairedPlayers.has(p._id.toString()));

    // 给予轮空的选手加0.5分
    for (const player of unpairedPlayers) {
      player.mcMahonScore = (player.mcMahonScore || 0) + 0.5;
      await player.save();
    }

    // 批量创建所有对局
    const savedMatches = await Match.create(matchesData);
    console.log('All matches saved:', savedMatches.map(m => m._id));
    ...
    console.log('New round data:', newRound);
    ...
    console.log('Updated tournament:', {
      roundsCount: updatedTournament.rounds.length,
      lastRound: updatedTournament.rounds[updatedTournament.rounds.length - 1]
    });

    // 再次验证数据
    const finalCheck = await Tournament.findById(tournamentId)
      .populate({
        path: 'rounds.matches',
        populate: ['player1', 'player2']
      })
    ...
    console.log('Final check - tournament rounds:', 
      finalCheck?.rounds.map(r => ({

...
const TournamentMatches = () => {
  ...
  const handleEndTournament = () => {
    ...
    if (tournament.format === 'ROUNDROBIN') {
    ...
    } else if (tournament.format === 'MCMAHON') {
      // 1. 检查是否已完成足够轮数（通常是4-5轮）
      const minRounds = 2;  // 测试阶段设置为2轮
      const hasEnoughRounds = tournament.rounds.length >= minRounds;
      canEndTournament = canEndTournament && hasEnoughRounds;
      ...
      // 3. 检查是否有明确的胜者（积分最高的选手）
      const playerScores = tournament.players
        .map(p => ({ player: p, score: p.score || 0 }))
        .sort((a, b) => b.score - a.score);
      
      const hasUniqueWinner = playerScores.length > 0 && 
        (playerScores.length === 1 || playerScores[0].score > playerScores[1].score);
      
      canEndTournament = canEndTournament && hasUniqueWinner;

      message += `\n3. 胜者确定要求：${hasUniqueWinner ? '✅' : '❌'}\n`;
      message += `   - 需要：有一个明确的胜者（积分最高且唯一）\n`;
      if (playerScores.length > 0) {
        message += `   - 当前最高分：${playerScores[0].score}分 (${playerScores[0].player.name})\n`;
        if (playerScores.length > 1) {
          message += `   - 第二高分：${playerScores[1].score}分 (${playerScores[1].player.name})\n`;
        }
      }
      message += `   - 结果：${hasUniqueWinner ? '满足' : '不满足，没有明确的胜者'}\n`;
    } else {

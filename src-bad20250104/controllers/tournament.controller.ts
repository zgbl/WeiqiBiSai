  ...
  async addPlayer(req: Request, res: Response) {
    try {
      ...
      tournament.playerScores.push({
        player: playerId,
        currentScore: tournament.initialScore || 0, // 使用比赛设置的初始分数
        initialScore: tournament.initialScore || 0,
        wins: 0,
        losses: 0,
        draws: 0,
        opponents: []
    ...
    try {
      ...
      for (const round of tournament.rounds) {
        ...
        if (match) {
          ...
          // 再调用 updateTournamentResults 更新选手得分
          console.log('Calling updateTournamentResults...');
          ...
          console.log('updateTournamentResults completed');
      ...
      // 返回更新后的比赛数据
      console.log('Fetching updated tournament...');
